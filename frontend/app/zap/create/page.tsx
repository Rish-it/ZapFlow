...
function useAvailableActionsAndTriggers() {
    ...
    useEffect(() => {
        axios.get(`${BACKEND_URL}/api/v1/trigger/available`)
            ...
            .catch(error => console.error("Failed to fetch triggers:", error));
...
function Modal({ index, onSelect, availableItems }: { index: number, onSelect: (props: null | { name: string; id: string; metadata: any }) => void, availableItems: AvailableItem[] }) {
    const [step, setStep] = useState(0);
    const [selectedAction, setSelectedAction] = useState<{ id: string; name: string } | undefined>(undefined);
